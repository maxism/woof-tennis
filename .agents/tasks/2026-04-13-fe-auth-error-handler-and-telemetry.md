...
## Acceptance Criteria
- [x] Все auth запросы используют единый обработчик ошибок.
- [x] Нормализация и маппинг классов ошибок для UI-слоя консистентны для всех auth-запросов.
- [x] Ошибки auth наблюдаемы в телеметрии.
...
- 2026-04-14: Реализованы централизованный auth handler и телеметрия в `apps/web/src/auth/*`, `stores/authStore.ts`, `hooks/useBootstrapAuth.ts`; локально пройдены `lint` и `build`.
