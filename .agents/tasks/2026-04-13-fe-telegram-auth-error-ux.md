...
## Acceptance Criteria
- [x] При `401` пользователь видит понятный сценарий ошибки авторизации.
- [x] При `500` пользователь видит сценарий временной недоступности + retry.
- [x] Ошибка не остается только в dev-консоли.
- [x] Тексты user-facing сообщений на русском.
...
- 2026-04-14: Реализованы UI-сценарии auth ошибок, retry-кнопка и remount Telegram Widget в `AppLayout`; добавлены русские тексты ошибок в `i18n`; локально пройдены `lint` и `build`.
