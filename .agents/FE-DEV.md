...
- Конфигурация клиента: только публичные переменные с префиксом `VITE_*` в `apps/web/.env`. Не ожидать общий `.env` с API и не класть серверные секреты в фронтенд-бандл.
