...
| Английский — комплиментарный | Поддерживается через словари локализации как fallback и для будущей i18n-экспансии |
| Two themes by default | Каждый экран поддерживает светлую и тёмную тему (Telegram theme aware) |
| Минимализм и ясность | Интерфейсы проектируются без визуальной перегрузки: короткие тексты, чистая иерархия, один главный CTA |
...
## MVP IA vNext (принято для UX-пакета)

- Нижняя навигация упрощена до двух разделов: `Главная` и `Профиль`.
- `Главная` объединяет обзор событий и основное действие через CTA `Создать`.
- `Тренеры` и `Игра` временно исключены из tabbar, чтобы убрать пересечение сценариев.
- `Уведомления` доступны в профиле и дублируются в Telegram.
